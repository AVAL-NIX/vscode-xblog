...
export class xblog {
   ...
   * @param isCheck 
   */
  validateData(isCheck: boolean): any {
    let r = new R();
    // 获取内容
  ...
  publicAritle() {
    ...
    this.requestApi(this.publicApi, data, (body: any) => {
      ...
      activeText.edit((editBuilder: { replace: (arg0: vscode.Range, arg1: string) => void; insert: (arg0: vscode.Position, arg1: string) => void; }) => {
        let meta = genMetaHeader(body.data.sign, body.data.title, body.data.channel, body.data.tags);
        if (text.startsWith("---")) {
          editBuilder.replace(new Range(new Position(0,0), new Position(5, data.length)), meta);
        } else {
...
---
`;
function genMetaHeader(sign: string, title: string, channel: any, tags: any) {
